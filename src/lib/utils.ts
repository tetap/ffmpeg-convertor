...
type GenerateFFmpegCommandType = (params: GenerateGMParams) => {
  command: string;
  commandParts: CommandPartsType;
};
export const generateFFmpegCommand: GenerateFFmpegCommandType = function (
  params
) {
  // 设置默认值
  const defaults = {
  ...
  const commandParts = [
    "ffmpeg",
    ` ${options.timeRange} `,
    `-i ${encodeURIComponent(options.input)}`,
    `-b:v ${options.bitrate}`,
  ...
  const _execute_parts = commandStr
    .replaceAll('"', "")
    .split(" ")
    .filter((i) => !!i);
